...
namespace GaussianSplatting.Editor
{
    ...
    public class GaussianSplatImporter : ScriptedImporter
    {
        ...
        public override unsafe void OnImportAsset(AssetImportContext ctx)
        {
            ...
            // Apply quality level if not custom
            if (m_Quality != DataQuality.Custom)
                ApplyQualityLevel();
            ...
            GaussianSplatAsset.CameraInfo[] cameras = LoadJsonCamerasFile(assetPath, m_ImportCameras);
            ...
            // Generate binary data as byte[]
            byte[] chunkBytes = null;
            if (IsUsingChunks)
                chunkBytes = CreateChunkBytes(inputSplats, ref dataHash);
            byte[] posBytes = CreatePositionsBytes(inputSplats, ref dataHash);
            byte[] otherBytes = CreateOtherBytes(inputSplats, splatSHIndices, ref dataHash);
            byte[] colorBytes = CreateColorBytes(inputSplats, ref dataHash);
            byte[] shBytes = CreateSHBytes(inputSplats, clusteredSHs, ref dataHash);
